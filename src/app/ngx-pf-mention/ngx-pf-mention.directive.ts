...
export class NgxPfMentionDirective {
      ...
      $event => {
        if (!this.isOpenDialog) {
          if ($event.data === this.ngxPfMentionKey) {
            this.loadComponent(this.ngxPfMention, this.ngxPfMentionFactory, $event);
  ...
  loadComponent(component: ComponentRef<any>, factory: NgxFactory, $event) {
    ...
    const componentFactory = this.componentFactoryResolver.resolveComponentFactory(NgxPfMentionDialogComponent);
  ...
  setMention(value) {
    ...
    this.inputEle.target.value = `${beforeStr} @${value[this.ngxPfMentionProperty]} ${afterStr}`;
  }
